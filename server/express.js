...
module.exports =  function(app) {
  ...
  const bodyParser    = require('body-parser'), // ENABLE FORM DATA,
        ...
        shared        = require('../config/shared.js'),
        webpack       = require('webpack'),
        webpackConfig = require(config.dir.config.webpack),
        util          = require('../lib/util-server'),
        logger        = util.logger;
  ...
  // SASS
  if (shared.engines.css === 'sass') {
    let output = 'compressed';
    if (shared.prettify) output = 'extended';
  ...
  // STYLUS
  } else if (shared.engines.css === 'stylus') {
    let compress = false;
    if (shared.prettify) compress = true;
    app.use(require('express-stylus')({
      src: './public/styles',
      dest: './public',
      use: [require('nib')()],
      import: ['nib'],
